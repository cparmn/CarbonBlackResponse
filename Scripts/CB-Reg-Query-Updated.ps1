 param
        ...
        [Parameter(
            Mandatory=$True,
            HelpMessage='Please enter a OU to Search Example:OU=OUName,DC=DomainName,DC=ToplevelDomain')]
        [ValidateNotNull()]
...
Retrieves Carbon Black Client registry keys from Domain joined computer that have logged in, in the past 30 days. 
...
.\CB-Reg-Query.ps1  -OU 'OU=Workstations,DC=Domain,DC=TopLevel' -KeyValue https://cbresponse.domain.org -KeyName SensorBackendServer
 OR
.\CB-Reg-Query.ps1  -OU 'OU=Workstations,DC=Domain,DC=TopLevel' -KeyName SensorBackendServer
#>
...
##Error Logging
        ...
        #Test
        #$AD_Workstation = Get-ADComputer -Filter {Name -eq 'cfg4236' -or Name -eq 'cfg22569' -or Name -eq 'cfg22568'-or Name -eq 'cfg4236-vm3' -or Name -eq 'cfg4236-vm1'  } -Properties Name,LastLogonDate -SearchBase $OU | Where { $_.LastLogonDate -GT (Get-Date).AddDays(-30) } | Select-Object Name
        
        foreach ($Computer in $AD_Workstation)
        {
            write-host 'Currently Running command on'$Computer.Name
            $Key=Invoke-command -ComputerName $Computer.Name {Get-ItemProperty -Path HKLM:\SOFTWARE\CarbonBlack\config -Name $Using:KeyName | select-object $Using:KeyName } -ErrorVariable ErrMsg 2> $null
            #$Key=Invoke-command -ComputerName $Computer.Name {Get-ItemPropertyValue -Path HKLM:\SOFTWARE\CarbonBlack\config -Name $Using:KeyName} -ErrorVariable ErrMsg 2> $null
            if(!$?)
            {
            $ErrMsg | Add-Content -Path $ErrorLog
            write-host "Failed to connect to"$Computer.Name"check File"$ErrorLog 
            }
            if ($Key.$KeyName -ne $KeyValue)
            {
                $CB_BAD_RegKey[$Computer.Name] = $Key.$KeyName
            }
            else
            {
                $CB_GOOD_RegKey[$Computer.Name] = $Key.$KeyName
            }
        }
...
CB-Query -OU $OU -KeyValue $KeyValue -KeyName $KeyName
# SIG # Begin signature block
# MIIZMAYJKoZIhvcNAQcCoIIZITCCGR0CAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU7g/jz/qrBy2I6x4pOGONQPXz
# ufegghQ+MIIEmTCCA4GgAwIBAgIPFojwOSVeY45pFDkH5jMLMA0GCSqGSIb3DQEB
...
# CQQxFgQU78rzoy2WA5oqJiorz0/atxk/Af0wDQYJKoZIhvcNAQEBBQAEggEAPMb8
# KK2xnMs3BGeyC47lFNTg+8yUPGgryVaXV5YwAdRGI9Zbjp4+Y7zNyDF7IFAp/LWu
# S3zkuAilW6+PVSQ9TMvRqSDG89RbbwWiLTBqbI+xSMuaT6fK6gm6RY/Y5Z5hYlLO
# CpqbH1gYStMbbnPN2VB7MITDkVs3sV+MJI4oCVW2fLg3vUiEhNYIUTx0NoAONr9C
# G8lgwCDamZc5IvtDG6vXikx/YdkTFewenOb+/gfzaBhAHoCpr1AaopwpO+ZCoXW8
# VHFMy6W0yZFfIM6/gVDhnFm5DrG4LVvgfZ5rjvbkTbjfLEaPRNpSYrioZBXyO+8t
# VGGJHSOTDmXizmPUTKGCAkMwggI/BgkqhkiG9w0BCQYxggIwMIICLAIBATCBqTCB
# lTELMAkGA1UEBhMCVVMxCzAJBgNVBAgTAlVUMRcwFQYDVQQHEw5TYWx0IExha2Ug
# Q2l0eTEeMBwGA1UEChMVVGhlIFVTRVJUUlVTVCBOZXR3b3JrMSEwHwYDVQQLExho
# dHRwOi8vd3d3LnVzZXJ0cnVzdC5jb20xHTAbBgNVBAMTFFVUTi1VU0VSRmlyc3Qt
# T2JqZWN0Ag8WiPA5JV5jjmkUOQfmMwswCQYFKw4DAhoFAKBdMBgGCSqGSIb3DQEJ
# AzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTE4MDcxMjIwNDkwNFowIwYJ
# KoZIhvcNAQkEMRYEFPoY7dZ9bNPB+EKZu83kllmrWpVRMA0GCSqGSIb3DQEBAQUA
# BIIBAFlGiKo3i6lqQA9KwZdBZgtERZakGcIoxw0SamCZrNY55ele8Pcf6WhZv+0t
# Mhgow9lJ1g08wbEQYrhbqENiIjJ3zvK1zRgMQly/xXB/fKPFaMYFagKT5Jd3/VMu
# m/AhkTDofa1aMyUGKM+d29BJQ8RQks/c9CqZgWu6V2rfB8/dYsbbaHD0/QFDwaQa
# MCEMvm8b9jqWfUsIlSkYhJwuxVUwzdckhBWxPKFDmpIBMydKk6FQOvVva/WHHs/S
# FTvVZmz5COSzqpPkwm8fPX5X3xjqrkgidDUjmv0HuXnJzUJtIQvs0GF70VG9Iwha
# 8Hp/9pqsxHJNMmjtGJNPHIC7ygE=
