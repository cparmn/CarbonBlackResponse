...
def storage_type(stor_type,size):
	...
	if type(stor_type) == dict:
		...
		try:
			...
			logging.warning("Not enough space on drive without any archives to delete, current size available: {} ".format(hbytes(size)))
			sys.exit()
			return 30
...
#This is where we'll need to do it with the os instead of the partition API calls OLD_COLD = min(COLD)
	...
	if DELETE_WARM == '10':
		...
		if DELETE_COLD == '10':
			logging.info("More than 600GB free in Cold Storage, Cleanup not required")
		else:
